...
const acceptOwnerRequestService = async (requestId, role) => {
  ...
  try {
      ...
      throw new AppError("Failed to approve the request", 500, { details: error.message });
  }
};
...
const RequestToBecomeRenterService = async (userId) => {
  ...
  const requestExists = await OwnersRentalRequest.findOne({where:{user_id : userId}})
  
  if (requestExists) throw new AppError("Users cant request more than once ", 403)
  if (!user) {
...
const getAllOwnersRequestsService = async () => {
  try {
      ...
      throw new AppError("Unable to load requests", 500, { details: error.message });
  }
};
